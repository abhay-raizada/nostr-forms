...
export const NumberConstraints: React.FC<NumberConstraintsProps> = ({onConstraintsChange}) => {
    const [error, showError] = React.useState(false)
    const maxRef = React.useRef<InputRef | null>()
    const minRef = React.useRef<InputRef | null>()
    const handleOnChange = () => {
        const maxValue = (maxRef.current?.input?.value) ? Number((maxRef.current?.input?.value)) :undefined
        const minValue = (minRef.current?.input?.value) ? Number((minRef.current?.input?.value)) :undefined
        ...
        <Row justify={'space-between'} >
            ...
            <Row>
                    ...
                    <Input
                        ...
                        type={'number'}
                        onChange={({target: {value}}) => {handleOnChange()}}
                    />
                    </Form.Item>
            </Row>
            <Row>
